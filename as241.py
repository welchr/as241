...
def ninv(p):
  SPLIT1, SPLIT2, CONST1, CONST2 = np.array([0.425,5.0,0.180625,1.6],dtype=np.float128)
  ...
  if np.abs(q) < SPLIT1:
    ...
    if r < np.finfo(np.float128).min:
      raise ValueError("p-value outside of range")
    ...
    if r > 0.0:
      ...
      # original code was the following, but HUGE_VAL was undefined: 
      # x = HUGE_VAL; 
      raise ValueError
